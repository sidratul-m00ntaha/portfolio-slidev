...
var _chunkQCIRGPRIcjs = require('./chunk-QCIRGPRI.cjs');
...
function createMarkdown(options) {
  ...
  const setupPromise = (async () => {
    const { default: MarkdownIt } = await Promise.resolve().then(() => _interopRequireWildcard(require("markdown-it-async")));
    const md = MarkdownIt({
      html: true,
      linkify: true,
      typographer: true,
    ...
    md.use(_plugincomponent.componentPlugin, options.componentOptions);
    if (options.frontmatter || options.excerpt) {
      md.use(_pluginfrontmatter.frontmatterPlugin, {
  ...
  return async (id, raw) => {
    ...
    raw = raw.trimStart();
    raw = await _asyncNullishCoalesce(await _optionalChain([transforms, 'access', _2 => _2.before, 'optionalCall', _3 => _3(raw, id)]), async () => ( raw));
    const env = { id };
    ...
    html = await _asyncNullishCoalesce(await _optionalChain([transforms, 'access', _4 => _4.after, 'optionalCall', _5 => _5(html, id)]), async () => ( html));
    ...
    if (options.frontmatter) {
      ...
      scriptLines.push(...await _optionalChain([transforms, 'access', _6 => _6.extraScripts, 'optionalCall', _7 => _7(frontmatter, id)]) || []);
    }
    if (options.excerpt) {
...
var _require = typeof _chunkQCIRGPRIcjs.__require === "undefined" ? _module.createRequire.call(void 0, _chunkQCIRGPRIcjs.importMetaUrl) : _chunkQCIRGPRIcjs.__require;
...
var index_default = /* @__PURE__ */ _unplugin.createUnplugin.call(void 0, unpluginFactory);




exports.unpluginFactory = unpluginFactory; exports.index_default = index_default;
